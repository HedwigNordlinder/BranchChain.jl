...
rundir = "runs/branchchain_allatomIPA256_$(Date(now()))_$(rand(100000:999999))"
...
Flux.thaw!(opt_state.layers.main_to_sides)
Flux.thaw!(opt_state.layers.sides_to_main)
Flux.thaw!(opt_state.layers.side_chain_embedder)
Flux.thaw!(opt_state.layers.side_chain_rff_embedder)
Flux.thaw!(opt_state.layers.side_chain_decoder)
Flux.thaw!(opt_state.layers.side_chain_ipa_blocks)
Flux.thaw!(opt_state.layers.side_chain_scframes_ipa_blocks)
Flux.thaw!(opt_state.layers.side_chain_cond)
...
for epoch in 6:7
    ...
    for (i, ts) in enumerate(batchloader(; device = device))
        #Give it a whole epoch of training before unfreezing:
        if (epoch == 2)  && (i == 1)
            Flux.thaw!(opt_state)
            sched = burnin_learning_schedule(0.00001f0, 0.000250f0, 1.05f0, 0.999995f0);
        end
        ...
        l, grad = Flux.withgradient(model) do m
            ...
            #pair_aux_loss = aux_atom11_to_centroid_loss(frames, atom14, ts)
            pair_aux_loss = aux_atom11_loss_fast(frames, atom14, ts)
            @show l_loc, l_rot, l_aas, l_atom14, l_splits, l_del, pair_aux_loss
            l_loc + l_rot + l_aas + l_atom14 + l_splits + l_del + pair_aux_loss
        end
