...
rundir = "runs/branchchain_allatomIPA256_cooldown_$(Date(now()))_$(rand(100000:999999))"
mkpath("$(rundir)/samples")
mkpath("$(rundir)/vids")

#olddat = load(PDBSimpleFlatV2_500);
#dat = load(PDBAtom14);
dat = JLD2.load("/home/murrellb/BranchChain.jl/sandbox/pdb-atom14.jld2")["data"];
...
#model = BranchChainAA3(merge(BranchChain.BranchChainAA3(384, 6, 6, 256).layers, oldmodel.layers)) |> device;

sched = burnin_learning_schedule(0.00001f0, 0.000250f0, 1.05f0, 0.999995f0);

#NOTE: A MODEL IMPORTED LIKE THIS WONT HAVE THE DELETION_PAD AND X0_MEAN_LENGTH SET CORRECTLY IN ITS CONFIG SO THAT WILL NEED TO BE ADDED MANUALLY.
model_state = JLD2.load("/home/murrellb/BranchChain.jl/sandbox/runs/branchchain_allatomIPA256_2026-01-14_831230/model_epoch_7_current.jld", "model_state");
model = BranchChainAA3(384, 6, 6, 256);
Flux.loadmodel!(model, model_state);
model = model |> device;

#Ugh my model states are not re-loading.
#opt_state = JLD2.load("/home/murrellb/BranchChain.jl/sandbox/runs/branchchain_allatomIPA256_2026-01-14_831230/model_epoch_7_current.jld", "opt_state") |> device;
...
#=
ts = training_prep([1], dat, deletion_pad, X0_mean_length, train_ff) |> device;
sc_frames = nothing
ts.t
...
aux_atom11_loss_fast(frames, atom14, ts)
a, b = aux_atom11_loss(frames, atom14, ts)
ts.t
=#
...
for epoch in 1:1
    for (i, ts) in enumerate(batchloader(; device = device))
        #if (epoch == 1) && (i == 1000)
        #    sched = linear_decay_schedule(sched.lr, 0.000000001f0, 2800)
        #end
        sc_frames = nothing
        ...
        l, grad = Flux.withgradient(model) do m
            frames, aa_logits, atom14, count_log, del_logit = m(ts.t', ts.Xt, ts.chainids, ts.resinds, ts.hasnobreaks, ts.chain_features, sc_frames = sc_frames)
        ...
        if mod(i, 5000) == 1000
            for v in 1:3
                try
                    ...
                    design(model, template[1], template[3], template[4], sampling_ff; vidpath = vidpath, device = device, path = "$(rundir)/samples/$(sampname).pdb", X0_mean_length = 0, deletion_pad = 1.1)
                catch
