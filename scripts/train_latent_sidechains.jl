...
rundir = "runs/branchchain_latentsc1_$(Date(now()))_$(rand(100000:999999))"
...
for v in 4:6
    ...
    feature_table[feature_table.pdb_id .== "7F5H",:]
    template = compoundstate(merge(dat[172742], (;latent_sc = latent_side_chains[172742])), mask_override = dat[172742].chainids .== 2)
    design(model, template[1], template[3], template[4], sampling_ff; steps = 500, vidpath = vidpath, device = device, path = "$(rundir)/samples/$(sampname).pdb", side_chain_decoder = cpu_decoder)
end
