...
Flux.thaw!(opt_state.layers.side_chain2x_embedder)
Flux.thaw!(opt_state.layers.sc_side_chain_embedder)
Flux.thaw!(opt_state.layers.sc_side_chain2x_embedder)
...
for epoch in 1:10 #Test run
    ...
    for (i, ts) in enumerate(batchloader(; device = device))
        ...
        l, grad = Flux.withgradient(model) do m
            ...
            softclamp(l_loc) + softclamp(l_rot) + softclamp(l_aas) + softclamp(l_atom14) + softclamp(l_splits) + softclamp(l_del) #+ pair_aux_loss
        end
